...
//     char* name;
//     int count_arguments;
//     struct TempArgs* next;
// } TempArgs;

// // Head of the linked list
// TempArgs* head = NULL;

// TempArgs* find_temp_arg(const char* name) {
//     TempArgs* current = head;
//     while (current != NULL) {
...
//     TempArgs* temp = find_temp_arg(name);
//     if (temp == NULL) {
//         // Create a new entry if it doesn't exist
//         temp = (TempArgs*)malloc(sizeof(TempArgs));
//         temp->name = strdup(name);
//         temp->count_arguments = 1;
//         temp->next = head;
//         head = temp;
//     } else {
//         // If it exists, increment the count
//         temp->count_arguments++;
//     }
// }

// // Function to get the number of arguments for a given name
// int get_number_of_temp_args(const char* name) {
//     TempArgs* temp = find_temp_arg(name);

//     if (temp == NULL) {
//         return -1; // Return -1 if the name doesn't exist
//     }
//     return temp->count_arguments;
// }

// // Function to delete a TempArgs by name
// void delete_temp_arg(const char* name) {
//     TempArgs* current = head;
//     TempArgs* previous = NULL;

//     while (current != NULL) {
//         if (strcmp(current->name, name) == 0) {
//             if (previous == NULL) { // Deleting the head of the list
//                 head = current->next;
//             } else { // Deleting a non-head element
...
void increase_count(const char* name) {
    ...
    printf("Function not found to increase count.\n");
}
...
%type <str> use_of_comp_variable use_comp_variable_with_period dot
...
//         // if (!func || (func->has_arguments != 99 && num_of_args_in_function_in_func_call != func->has_arguments)) {
...
//         } 
//         printf("%d\n", func->has_arguments);
//         printf("%d\n", num_of_args_in_function_in_func_call);

//         if (func->has_arguments != 99 && num_of_args_in_function_in_func_call != func->has_arguments) {
...
//====================================================================================
  ...
  {
      char* comp_name = find_comp_type($1);
...
dot:
    %empty                                                  { $$ = ""; } 
  | dot func_call                                           { $$ = template("%s%s", $1, $2); }
  | dot use_of_comp_variable                                { $$ = template("%s%s", $1, $2); }
;
