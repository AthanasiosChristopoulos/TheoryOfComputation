...
commands:
    KW_IF '(' expr ')' ':' { indentation_level++; is_a_loop = 0;} 
    stmts 
    {
        indented_stmts = indent_statements($7, indentation_level);
        indentation_level--;
        indented_stmts = strcat(indented_stmts, "}\n");
    }
    else_statement KW_ENDIF {
        $$ = template("%s (%s) {\n%s%s\n", $1, $3, indented_stmts, $9);
    }
    //==============================================================================================================================================================
    | KW_WHILE '(' expr ')' ':' {indentation_level++; is_a_loop = 1;}
    stmts 
    {
    ...
    {
        if(for_has_step == 1){
            temp5 = template("for (int %s = %s; %s < %s; %s =+ %s) {\n", $2, $5, $2, $7, $2, $8); 
        } else {
            temp5 = template("for (int %s = %s; %s < %s; %s =+ %s) {\n", $2, $5, $2, $7, $2, $8); 
        }
...
stmts:
    %empty                                        { $$ = ""; }
  | stmts loop_thing ';'                           
  { if (is_a_loop != 1) {  yyerror("This isnt a loop"); YYABORT;} $$ = template("%s%s;\n", $1, $2);} 
...
function_definition_comp:
  KW_DEF IDENTIFIER '(' arguments ')' return_type_comp ':' body KW_ENDDEF
  {
    ...
    indented_stmts_5 = indent_statements($8, indentation_level);

    char* function_follow_comp = template("%s %s(struct %s *self%s%s) {\n%s}\n", $6, $2, comp_name,strlen($4) == 0 ? "":", " , $4, indented_stmts_5 );
