...
%type <str> complicated_types function_definition_comp return_type_comp body_comp
...
complicated_types:
  ...
  KW_COMP IDENTIFIER { inside_comp = 1; comp_name = $2;}
  ':' declaration_comp body_comp KW_ENDCOMP 
  { 
    add_identifier($2); 
    for (int i = 0; i < function_list_count; i++) {
...
variable_definition_comp:
  variable_list_comp ':' KW_VARIABLE_TYPE            { $$ = template("%s %s", $3, $1); }
;

variable_list_comp:
    '#' IDENTIFIER                              { add_identifier($2); $$ = template("%s", $2); }
  | variable_list_comp ',' '#' IDENTIFIER       { add_identifier($4); $$ = template("%s, %s", $1, $4); }
;
...
body_comp: 
  ...
  | body_comp expr ';'                         { assignment_made = 0; $$ = template("%s    %s;\n", $1, $2); }
  | KW_RETURN expr ';'                         { body_has_return_type = 1; $$ = template("    %s %s;\n", $1 , $2 ); }
;
...
expr:
  ...
  | NUMBERS                             { $$ = $1; }
  |'#' IDENTIFIER                       { if (inside_comp == 0) {yyerror("Not in comp"); YYABORT;} else { $$ = template("%s", $2);} }
  |'#' IDENTIFIER '[' expr ']'          { if (inside_comp == 0) {yyerror("Not in comp"); YYABORT;} else { $$ = template("%s[%s]", $2, $4);} }
  | IDENTIFIER                          { $$ = $1; }
